...
-- Если выражение или запрос в отдельных скобках не умещаются на одной строке, 
-- содержимое скобок начинается с новой строки, с одним отступом
	...
	select
		...
		,case
			-- При написании конструкции с  case , необходимо, чтобы  when  был под  case  с 1 отступом,  
			-- then  с 2 отступами, дополнительные условия  and  с 3 отступами, для визуального понимания написанных условий
			when c.ID is null 
	...
	left join dbo.Customer as c on c.UID_DS = cs.UID_DS_Customer
		and c.ID_mapping_DataSource = 1
	-- Дополнительные условия переносятся на следующую строку с 1 отступом
